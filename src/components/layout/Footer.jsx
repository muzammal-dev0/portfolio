    ...
    <footer className="bg-gray-900 text-white py-8">
      <div className="container mx-auto px-4">
        ...
        <div className="mt-8 flex justify-center">
          <Dock className="bg-white/10 border-white/20">
            {socialLinks.map((link, index) => (
              <DockIcon key={index} className="bg-transparent hover:bg-white/10">
                <a
                  href={link.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={link.label}
                  className="flex items-center justify-center w-full h-full text-white hover:text-white/80 transition-colors"
                >
                  {link.icon}
                </a>
              </DockIcon>
